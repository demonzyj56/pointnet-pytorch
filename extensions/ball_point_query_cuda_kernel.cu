    ...
    for (auto index = blockIdx.x * blockDim.x + threadIdx.x; index < n; index += blockDim.x * gridDim.x) {
        ...
        for (auto i = 0; i < num_points; ++i) {
            ...
            if (dist < static_cast<scalar_t>(radius*radius)) {
                group_idx[index*max_samples+cur_idx] = static_cast<scalar_t>(i);
                cur_idx++;
            }
            if (dist < min_val) {
                min_val = dist;
        ...
        if (cur_idx < max_samples) {
            for (auto i = cur_idx; i < max_samples; ++i) {
                group_idx[index*max_samples+i] = static_cast<scalar_t>(min_idx);
            }
        }
    }
}
...
std::vector<at::Tensor> bpq_cuda_forward(at::Tensor pcs, at::Tensor centroids, 
    ...
    auto group_idx = at::zeros({batch_size, num_centroids, static_cast<long>(max_samples)}, pcs.options());
    ...
    AT_DISPATCH_FLOATING_TYPES(pcs.type(), "bpq_cuda_forward", ([&] {
        bpq_cuda_forward_kernel<<<blocks, threads>>>(
            ...
            pcs.data<scalar_t>(), centroids.data<scalar_t>(),
            group_idx.data<scalar_t>()
        );
    }));
    return {group_idx};
}
