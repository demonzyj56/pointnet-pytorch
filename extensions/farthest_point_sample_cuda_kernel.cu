...
// Fix some bugs.
template <typename scalar_t>
__global__ void fps_kernel_correct(int batch_size, int num_points, int num_centroids, const scalar_t *pcs,
        int64_t *centroid_idx) {
    // shared memory
    extern __shared__ float sdata[];
    float *dists = sdata;
...
void fps_cuda_correct(at::Tensor pcs, at::Tensor out) {
    ...
    int smem_size = (sizeof(float) + sizeof(int64_t) + sizeof(bool)) * threads;
    AT_DISPATCH_FLOATING_TYPES(pcs.type(), "fps1_kernel", ([&] {
        fps_kernel_correct<<<batch_size, threads, smem_size>>>(batch_size, num_points, num_centroids,
                pcs.data<scalar_t>(), out.data<int64_t>());
    }));
}
